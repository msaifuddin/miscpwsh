...
function Draw-Banner {
    ...
    # Simple Text Banner
    $bannerText = "User and Device Search Tool"
    $bannerLength = $bannerText.Length + 4
    Write-Host ("=" * $bannerLength) -ForegroundColor Cyan
...
do {
    ...
    if ($global:isConnected) {
        while ($global:isConnected) {
            Draw-Banner $global:userDisplayName
            Write-Host "----------------------------------------" -ForegroundColor Yellow
            Write-Host "1. Search [U]sers"
            Write-Host "2. Search [D]evices"
            Write-Host "3. [E]xit"

            Write-Host "`nOptions: [1] Search Users, [2] Search Devices, [3] Exit"
